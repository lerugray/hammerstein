...
Effort estimates assume Ray-as-solo-dev with AI-pair-programmer at ~30-50%
time allocation.
...
- **Concrete v0 benchmark-question candidates** drawn from real Ray-vs-
  Claude conversation history (§ "v0 benchmark suite" below).
- **Locked eval rubric** — Ray-rated 1-5 across three explicit axes.
...
- **Eval harness:**
  ...
  - Quality scoring: Ray-rated **1-5 per response** on three axes:
...
A user (currently just Ray) wanting Hammerstein-style strategic reasoning
should be able to:
...
4. On the 5-10 benchmark questions, Hammerstein-on-cheap-model produces
   responses that are ≥60% as useful as Claude (Ray-rated). 80% would be
   a clear ship.
...
from real Ray-vs-Claude strategic conversations where the response was
...
2. **The 2026-04-21 fleet-walk-session "why GS over Polsia"
   articulation** (the operator's `[GS-Ray:2026-04-21]` entry). Strategic-
...
7. **A novel question (TBD)** — a strategic-reasoning question Ray
   has not previously asked Claude, surfaced fresh during the eval
...
4. On the 8 benchmark questions, Hammerstein-on-cheap-model produces
   responses that are ≥60% as useful as Claude (Ray-rated). 80% is
   a clear ship.
5. Per-call cost: $0 (local) to $0.01 (cloud).
6. Full framework legibility: a future Claude or fresh contributor can
   read the system prompt + corpus + harness and understand exactly how
