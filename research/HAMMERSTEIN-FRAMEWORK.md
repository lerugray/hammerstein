...
- `[GS-Ray:<date>]` — `generalstaff/docs/internal/Hammerstein Observations Log.md`
...
[`Hammerstein log ground rules`, see `[GS-Ray:header]`,
...
- **Surrogate-brain draft** [`[CDNA-Log:2026-04-14]`]. Asked to scope
  a "Ray-surrogate brain" for GS, Claude pushed back on the ambitious
...
3. The structural fix **compounds across future runs**: the rule
   ...
   [`[GS-Ray:2026-04-16]`, `[CDNA-Log:2026-04-13]`].
...
articulation [`[GS-Ray:2026-04-21]`]:
...
dimension [Claude's same-day reply, `[GS-Ray:2026-04-21]` Claude
...
some of what reads as "taste work" by querying that encoded-Ray
[`[GS-Ray:2026-04-21]`]. **This is not a refutation of the BYOI
...
the operator floated extracting his conversation logs into a "Ray-surrogate
...
- **Game-design framing is testable when external users encounter
  ...
  (suggests the vocabulary was a Ray-specific affordance, not a
...
- **Architecture transfers**: the rule is encoded in
  ...
  is not a prompt — it is a Ray-facing artifact the dispatcher
  produces.
...
- **TWAR-PC and Greater-Than-Alexander** (per `README.md`) are
  ...
  also Ray-managed projects in similar idiom. The structurally-
...
compounding mechanism's signature [`[GS-Ray:2026-04-15]`]:
...
12. **Push back when asked to imagine on the user's behalf in
    ...
    domains for Ray: purpose / feel / aesthetic / strategic
    direction. (§3.5)
...
- **A user who is hostile to the framework.** All current users are
  Ray, who designed it. A user actively trying to push Hammerstein-
...
structural fix, and bring the proposed revision through Ray.
