...
For prompt design: the system prompt's job is **identity-framing**, not just rule-listing. The Hammerstein system prompt at v0 frames the model as *"a strategic-reasoning collaborator working within the operator's Hammerstein framework, surfacing verification questions and structural fixes, leaving imagination + taste + direction with Ray."* That identity frames the generalization, not the rule list.
